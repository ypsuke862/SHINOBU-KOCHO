...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    if (!args[0] || isNaN(args[0])) return m.reply(`_Ingresa un número que represente el número de días._\n\n*• Ejemplo:*\n*${usedPrefix + command}* 30`)
    ...
    let teks = `_Se estableció los días de vencimiento para \n*${await conn.getName(who)}*_\n\n*Durante:* ${args[0]} Días\n\n*Cuenta regresiva :* ${msToDate(global.db.data.chats[who].expired - now)}`
    m.reply(teks)
}
